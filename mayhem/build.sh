...
#       §6.2 item 16's `git clean -ffdX` (simulating rlenv's real patch-grader clean) removes every
#       extracted/cloned subproject dir (gitignored) — most of these are wrap-git subprojects
#       pinned to `revision = HEAD`, so meson can only re-satisfy them with a fresh `git clone`,
#       which the offline rebuild can't do. SUBPROJECTS_CACHE holds the whole subprojects/ tree
#       mayhem/Dockerfile snapshotted right after the first (networked) build.sh run; restoring it
#       wholesale (rather than special-casing each subproject's wrap type) sidesteps the network
#       entirely on a clean-tree rebuild. ───────────────────────────────────────────────────────
SUBPROJECTS_CACHE=/opt/phosphor-subprojects-cache
if [ -d "$SUBPROJECTS_CACHE" ] && [ ! -d subprojects/sdbusplus ]; then
  cp -r "$SUBPROJECTS_CACHE"/. subprojects/
fi
...
#       sanitizer runtime. -Db_lto=false overrides the project's own default_options (b_lto=true):
#       LTO's cross-TU IR for phosphor-dbus-interfaces' large generated D-Bus interface surface,
#       combined with ASan+UBSan instrumentation, OOMs the final `ar`/link step (observed >13G RSS).
#       We're fuzzing correctness, not shipping an optimized binary, so LTO buys nothing here. ──────
...
COMMON_OPTS=(
  ...
  # libuserlayer must stay ENABLED (upstream's own default for this `feature` option is auto).
  # We used to pass -Dlibuserlayer=disabled; upstream then made the user layer non-optional for the
  # main library — meson.build adds `userlayer_dep` to ipmid_pre and 'user_channel/user_layer.cpp'
  # to libipmi20_src UNCONDITIONALLY, while user_channel/meson.build only declares userlayer_dep
  # inside `if get_option('libuserlayer').allowed()`. Disabling it therefore fails configure with
  # `meson.build:258:4: ERROR: Unknown variable "userlayer_dep"`. Its deps (crypto, pam,
  # libsystemd, nlohmann_json, sdbusplus, phosphor-logging) are all present in the image.
